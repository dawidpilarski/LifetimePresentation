...
\only{
\begin{tikzpicture}[remember picture, overlay]
\draw[color=magenta, thick, <-] (2.6,5.3) to[out=0, in=160] node[above]{vptr write} (5.5,5);
\draw[color=blue, thick, <-] (4.6,6.3) to[out=0, in=70] node[right]{vptr read}(5.5,5);
\draw[color=violet, thick, <-] (5.5,5) to[out=-110, in=-180] (5.5, 4) node[right]{data race};
\end{tikzpicture}
}<4>
