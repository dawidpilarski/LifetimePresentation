...
\only{\alert{*In case of default construction of trivial type, there is no initialization performed}}<2>
...
\item[non-class types] when we expect it to end it's lifetime, 
\begin{itemize}
	\item when object exits the scope,
	\item delete expression,
	\item when temporary ends it's lifetime etc.
\end{itemize}
